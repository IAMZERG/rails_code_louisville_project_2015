  ...
  describe "PATCH update" do
    ...
    context "with a valid token" do
      ...
      it "updates the user's password" do
        ...
        patch :update, id: user.password_reset_token, user: { password: 'newpassword2', password_confirmation: 'newpassword2' }
        user.reload
        expect(flash[:notice]).to match(/was successful/)
        expect(user.password_digest).to_not eq(digest)
      end

      it "clears the password reset token for the user" do
        patch :update, id: user.password_reset_token, user: { password: 'newpassword2', password_confirmation: 'newpassword2' }
        user.reload
      ...
      it "sets the flash[:notice] to success" do
        patch :update, id: user.password_reset_token, user: { password: 'newpassword2', password_confirmation: 'newpassword2' }
        expect(flash[:notice]).to match(/was successful/)
      end
      it "redirects to the decklists path" do
        patch :update, id: user.password_reset_token, user: { password: 'newpassword2', password_confirmation: 'newpassword2' }
